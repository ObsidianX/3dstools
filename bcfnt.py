...
class Bcfnt:
    ...
    def read(self, filename):
        ...
        self._parse_header(data[:CFNT_HEADER_SIZE])
        position = CFNT_HEADER_SIZE
        if self.invalid:
            return
    ...
    def save(self, filename):
        ...
        data = struct.pack(CFNT_HEADER_STRUCT % self.order, magic, bom, CFNT_HEADER_SIZE, self.version, 0, 0)
        file_.write(data)
    ...
    def _parse_header(self, data):
        ...
        magic, bom, header_size, self.version, file_size, sections, unknown = struct.unpack(CFNT_HEADER_STRUCT % self.order, data)

        if magic not in CFNT_HEADER_MAGIC:
            print('Invalid CFNT magic bytes: %s (expected %s)' % (magic, CFNT_HEADER_MAGIC))
            self.invalid = True
            return
        ...
        if self.version not in VERSIONS:
            print('Unknown version: 0x%08x (expected 0x%08x or 0x%08x)' % (self.version, 0x03000000, 0x04000000))
            self.invalid = True
            return

        if header_size != CFNT_HEADER_SIZE:
            print('Invalid header size: %d (expected %d)' % (header_size, CFNT_HEADER_SIZE))
            blinh
            self.invalid = True
            return
        ...
        if self.debug:
            print('CFNT Magic: %s' % magic)
            print('CFNT BOM: %s (0x%x)' % (self.order, bom))
            print('CFNT Header Size: %d' % header_size)
            print('CFNT Version: 0x%08x' % self.version)
            print('CFNT File Size: %d' % file_size)
            print('CFNT Sections: %d\n' % sections)
...
if __name__ == '__main__':
    parser = argparse.ArgumentParser(description='BCFNT Converter Tool')
    ...
    group.add_argument('-l', '--little-endian', help='Use little endian encoding in the created BCFNT file (default)',
                       action='store_true', default=False)
    group.add_argument('-b', '--big-endian', help='Use big endian encoding in the created BCFNT file',
                       action='store_true', default=False)
    group = parser.add_mutually_exclusive_group(required=True)
    group.add_argument('-c', '--create', help='create BCFNT file from extracted files', action='store_true',
                       default=False)
    group.add_argument('-x', '--extract', help='extract BCFNT into PNG/JSON files', action='store_true', default=False)
    parser.add_argument('-f', '--file', metavar='bcfnt', help='BCFNT file', required=True)
    args = parser.parse_args()

    if args.extract and not os.path.exists(args.file):
        print('Could not find BCFNT file:')
        print(args.file)
        sys.exit(1)
    ...
    if args.create and os.path.exists(args.file) and not args.yes:
        print('BCFNT output file exists.')
        answer = prompt_yes_no('Overwrite? (y/N) ')
    ...
    if args.big_endian:
        ...
        order = '<'
    bcfnt = Bcfnt(load_order=order, verbose=args.verbose, debug=args.debug)

    if args.extract:
        bcfnt.read(args.file)
        if not bcfnt.invalid:
            bcfnt.extract()
    elif args.create:
        bcfnt.load(json_file)
        if not bcfnt.invalid:
            bcfnt.save(args.file)
